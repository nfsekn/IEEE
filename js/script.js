...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const animateElements = document.querySelectorAll('.about-item, .committee-card, .event-card');
...
document.querySelectorAll('.committee-card').forEach(card => {
    ...
    const features = card.querySelector('.committee-features');
    
    if (moreBtn && features) {
        let isExpanded = false;
        const originalHeight = features.scrollHeight;
        
        // Initially hide extra features (show only first 2)
        const featureItems = features.querySelectorAll('li');
        if (featureItems.length > 2) {
            for (let i = 2; i < featureItems.length; i++) {
...
document.querySelectorAll('.committee-card, .about-item, .event-card').forEach(card => {
...
function performSearch(query) {
    // This would search through committee information, events, etc.
